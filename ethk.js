...
async function handleMessage(msg) {
    ...
    if (
        ...
        (await process.core.ethkusers.create(sender)) ||
        //Or if they need to be notified...
        (await process.core.ethkusers.getNotify(sender))
    ) {
        ...
        await process.core.ethkusers.setNotified(sender);
        return;
    }
...
async function main() {
    ...
    process.settings = require("./ethk.json");
    //Load it's path separately so we can write to it without writing the path.
    process.settingsPath = path.join(__dirname, "ethk.json");
    ...
    commands = {
        ...
        b_ethk:             require("./commands/ethkbalance.js"),
