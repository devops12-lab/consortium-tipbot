...
async function createAddress() {
    ...
    //Save it to the disk.
    fs.writeFileSync(process.ethksettings.coin.keys + address + ".json", JSON.stringify(newWallet.toV3(""), null, 4));
    ...
    var approve = await web3.eth.accounts.signTransaction({
        to: process.ethksettings.coin.addresses.contract,
...
module.exports = async () => {
    //Init Web3.
    web3 = new web3c(process.ethksettings.coin.infura);
    ...
    contract = new web3.eth.Contract(abi, process.ethksettings.coin.addresses.contract);
    //Set the decimals and decimalsBN.
    decimals = process.ethksettings.coin.decimals;
    decimalsBN = BN(10).pow(decimals);

    //Set the master address.
    master = process.ethksettings.coin.addresses.wallet.toLowerCase();

    //Load every account.
    addresses = [];
    var filenames = fs.readdirSync(process.ethksettings.coin.keys);
    console.log("Adding addresses:")
    for (var file in filenames) {
        console.log("addresss " + file + " processing");
        var wallet = ethjsWallet.fromV3(fs.readFileSync(process.ethksettings.coin.keys + filenames[file]).toString(), "", true);
