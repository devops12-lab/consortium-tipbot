...
async function createAddress() {
    ...
    //Save it to the disk.
    fs.writeFileSync(process.xmr1settings.coin.keys + address + ".json", JSON.stringify(newWallet.toV3(""), null, 4));
    ...
    var approve = await web3.eth.accounts.signTransaction({
        to: process.xmr1settings.coin.addresses.contract,
...
module.exports = async () => {
    //Init Web3.
    web3 = new web3c(process.xmr1settings.coin.infura);
    ...
    contract = new web3.eth.Contract(abi, process.xmr1settings.coin.addresses.contract);
    //Set the decimals and decimalsBN.
    decimals = process.xmr1settings.coin.decimals;
    decimalsBN = BN(10).pow(decimals);

    //Set the master address.
    master = process.xmr1settings.coin.addresses.wallet.toLowerCase();

    //Load every account.
    addresses = [];
    var filenames = fs.readdirSync(process.xmr1settings.coin.keys);
    console.log("Adding addresses:")
    for (var file in filenames) {
        console.log("addresss " + file + " processing");
        var wallet = ethjsWallet.fromV3(fs.readFileSync(process.xmr1settings.coin.keys + filenames[file]).toString(), "", true);
