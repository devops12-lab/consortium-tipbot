...
//Definition of the table: `name VARCHAR(64), address VARCHAR(64), btcf VARCHAR(64), notify tinyint(1)`.
...
async function create(user) {
    ...
    //Create the new user, with a blank address, btcf of 0, and the notify flag on.
    await connection.query("INSERT INTO " + table + " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [user, "", "0", "0", "0", "0", "0", "0", "0", "0", "0"]);
    //Create the new user in the RAM cache, with a status of no address, btcf of 0, and the notify flag on.
    users[user] = {
        address: false,
        btcf: BN(0),
        notify: true
...
async function addBalance(user, amount) {
    ...
    //Add the amount to the btcf.
    var btcf = users[user].btcf.plus(amount);
    //Convert the btcf to the coin's smallest unit.
    btcf = btcf.toFixed(process.settings.coin.decimals);
    //Update the table with the new btcf, as a string.
    await connection.query("UPDATE " + table + " SET btcf = ? WHERE name = ?", [btcf, user]);
    //Update the RAM cache with a BN.
    users[user].btcf = BN(btcf);

    return true;
}

//Return the btcf for all users
async function getAllBalance() {
    table = process.settings.mysql.tips;
    rows = await connection.query("SELECT sum(btcf) as btcf FROM " + table);
    
    return BN(rows[0].btcf);
}

//Subtracts from an user's btcf.
async function subtractBalance(user, amount) {
    ...
    //Subtracts the amount from the btcf.
    var btcf = users[user].btcf.minus(amount);
...
//Returns an user's btcf
async function getBalanceBTCF(user) {
    return users[user].btcf;
}
