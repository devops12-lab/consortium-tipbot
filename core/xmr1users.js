...
//Definition of the table: `name VARCHAR(64), address VARCHAR(64), xmr1 VARCHAR(64), notify tinyint(1)`.
...
async function create(user) {
    ...
    //Create the new user, with a blank address, xmr1 of 0, and the notify flag on.
    await connection.query("INSERT INTO " + table + " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [user, "", "0", "0", "0", "0", "0", "0", "0", "0", "0"]);
    //Create the new user in the RAM cache, with a status of no address, xmr1 of 0, and the notify flag on.
    users[user] = {
        address: false,
        xmr1: BN(0),
        notify: true
...
async function addBalance(user, amount) {
    ...
    //Add the amount to the xmr1.
    var xmr1 = users[user].xmr1.plus(amount);
    //Convert the xmr1 to the coin's smallest unit.
    xmr1 = xmr1.toFixed(process.settings.coin.decimals);
    //Update the table with the new xmr1, as a string.
    await connection.query("UPDATE " + table + " SET xmr1 = ? WHERE name = ?", [xmr1, user]);
    //Update the RAM cache with a BN.
    users[user].xmr1 = BN(xmr1);

    return true;
}

//Return the xmr1 for all users
async function getAllBalance() {
    table = process.settings.mysql.tips;
    rows = await connection.query("SELECT sum(xmr1) as xmr1 FROM " + table);
    
    return BN(rows[0].xmr1);
}

//Subtracts from an user's xmr1.
async function subtractBalance(user, amount) {
    ...
    //Subtracts the amount from the xmr1.
    var xmr1 = users[user].xmr1.minus(amount);
...
//Returns an user's xmr1
async function getBalanceXMR1(user) {
    return users[user].xmr1;
}
