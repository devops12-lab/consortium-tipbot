...
//Definition of the table: `name VARCHAR(64), address VARCHAR(64), et3d VARCHAR(64), notify tinyint(1)`.
...
async function create(user) {
    ...
    //Create the new user, with a blank address, et3d of 0, and the notify flag on.
    await connection.query("INSERT INTO " + table + " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [user, "", "0", "0", "0", "0", "0", "0", "0", "0", "0"]);
    //Create the new user in the RAM cache, with a status of no address, et3d of 0, and the notify flag on.
    users[user] = {
        address: false,
        et3d: BN(0),
        notify: true
...
async function addBalance(user, amount) {
    ...
    //Add the amount to the et3d.
    var et3d = users[user].et3d.plus(amount);
    //Convert the et3d to the coin's smallest unit.
    et3d = et3d.toFixed(process.settings.coin.decimals);
    //Update the table with the new et3d, as a string.
    await connection.query("UPDATE " + table + " SET et3d = ? WHERE name = ?", [et3d, user]);
    //Update the RAM cache with a BN.
    users[user].et3d = BN(et3d);

    return true;
}

//Return the et3d for all users
async function getAllBalance() {
    table = process.settings.mysql.tips;
    rows = await connection.query("SELECT sum(et3d) as et3d FROM " + table);
    
    return BN(rows[0].et3d);
}

//Subtracts from an user's et3d.
async function subtractBalance(user, amount) {
    ...
    //Subtracts the amount from the et3d.
    var et3d = users[user].et3d.minus(amount);
...
//Returns an user's et3d
async function getBalanceET3D(user) {
    return users[user].et3d;
}
