...
//Definition of the table: `name VARCHAR(64), address VARCHAR(64), unix VARCHAR(64), notify tinyint(1)`.
...
async function create(user) {
    ...
    //Create the new user, with a blank address, unix of 0, and the notify flag on.
    await connection.query("INSERT INTO " + table + " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [user, "", "0", "0", "0", "0", "0", "0", "0", "0", "0"]);
    //Create the new user in the RAM cache, with a status of no address, unix of 0, and the notify flag on.
    users[user] = {
        address: false,
        unix: BN(0),
        notify: true
...
async function addBalance(user, amount) {
    ...
    //Add the amount to the unix.
    var unix = users[user].unix.plus(amount);
    //Convert the unix to the coin's smallest unit.
    unix = unix.toFixed(process.settings.coin.decimals);
    //Update the table with the new unix, as a string.
    await connection.query("UPDATE " + table + " SET unix = ? WHERE name = ?", [unix, user]);
    //Update the RAM cache with a BN.
    users[user].unix = BN(unix);

    return true;
}

//Return the unix for all users
async function getAllBalance() {
    table = process.settings.mysql.tips;
    rows = await connection.query("SELECT sum(unix) as unix FROM " + table);
    
    return BN(rows[0].unix);
}

//Subtracts from an user's unix.
async function subtractBalance(user, amount) {
    ...
    //Subtracts the amount from the unix.
    var unix = users[user].unix.minus(amount);
...
//Returns an user's unix
async function getBalance(user) {
    return users[user].unix;
}
