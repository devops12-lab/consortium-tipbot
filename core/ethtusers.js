...
//Definition of the table: `name VARCHAR(64), address VARCHAR(64), etht VARCHAR(64), notify tinyint(1)`.
...
async function create(user) {
    ...
    //Create the new user, with a blank address, etht of 0, and the notify flag on.
    await connection.query("INSERT INTO " + table + " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [user, "", "0", "0", "0", "0", "0", "0", "0", "0", "0"]);
    //Create the new user in the RAM cache, with a status of no address, etht of 0, and the notify flag on.
    users[user] = {
        address: false,
        etht: BN(0),
        notify: true
...
async function addBalance(user, amount) {
    ...
    //Add the amount to the etht.
    var etht = users[user].etht.plus(amount);
    //Convert the etht to the coin's smallest unit.
    etht = etht.toFixed(process.settings.coin.decimals);
    //Update the table with the new etht, as a string.
    await connection.query("UPDATE " + table + " SET etht = ? WHERE name = ?", [etht, user]);
    //Update the RAM cache with a BN.
    users[user].etht = BN(etht);

    return true;
}

//Return the etht for all users
async function getAllBalance() {
    table = process.settings.mysql.tips;
    rows = await connection.query("SELECT sum(etht) as etht FROM " + table);
    
    return BN(rows[0].etht);
}

//Subtracts from an user's etht.
async function subtractBalance(user, amount) {
    ...
    //Subtracts the amount from the etht.
    var etht = users[user].etht.minus(amount);
...
//Returns an user's etht
async function getBalanceETHT(user) {
    return users[user].etht;
}
