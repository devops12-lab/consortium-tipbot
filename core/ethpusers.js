...
//Definition of the table: `name VARCHAR(64), address VARCHAR(64), ethp VARCHAR(64), notify tinyint(1)`.
...
async function create(user) {
    ...
    //Create the new user, with a blank address, ethp of 0, and the notify flag on.
    await connection.query("INSERT INTO " + table + " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [user, "", "0", "0", "0", "0", "0", "0", "0", "0", "0"]);
    //Create the new user in the RAM cache, with a status of no address, ethp of 0, and the notify flag on.
    users[user] = {
        address: false,
        ethp: BN(0),
        notify: true
...
async function addBalance(user, amount) {
    ...
    //Add the amount to the ethp.
    var ethp = users[user].ethp.plus(amount);
    //Convert the ethp to the coin's smallest unit.
    ethp = ethp.toFixed(process.settings.coin.decimals);
    //Update the table with the new ethp, as a string.
    await connection.query("UPDATE " + table + " SET ethp = ? WHERE name = ?", [ethp, user]);
    //Update the RAM cache with a BN.
    users[user].ethp = BN(ethp);

    return true;
}

//Return the ethp for all users
async function getAllBalance() {
    table = process.settings.mysql.tips;
    rows = await connection.query("SELECT sum(ethp) as ethp FROM " + table);
    
    return BN(rows[0].ethp);
}

//Subtracts from an user's ethp.
async function subtractBalance(user, amount) {
    ...
    //Subtracts the amount from the ethp.
    var ethp = users[user].ethp.minus(amount);
...
//Returns an user's ethp
async function getBalanceETHP(user) {
    return users[user].ethp;
}
